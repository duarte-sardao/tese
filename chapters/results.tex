...
This chapter answers \textbf{RQ1 -- RQ3}, which aim to study ChatGPT's ability to identify, explain, and reveal semantic conflicts. % the latter with both our prompt, which provides an explanation, and state-of-the-art prompts, which do not.

\section[Research Question 1: Conflict Explanation]{RQ1: Can ChatGPT identify the presence of a semantic conflict in a merge commit, as well as explain it?}\label{sec:results:rq1}
...
This remained a general trend, resulting on the average value of 0.9 for metric C.

It is however worth addressing the exceptions: Cart (CM), which always yields a positive response and Cart (PF) as well as Antlr (PF), which always yield negative responses.
Immediately we can observe that both purely negative examples are of type ``Parallel Field''. When we analyse their responses, we see a pattern: the changes made to the branches 
...
We see examples of changes made in branch B being described as changes made in A, such as null checks.
\begin{response}
...
In contrast to these scenarios, which may be so subtle ChatGPT does not understand them as conflicts, we find subjects so complicated, the LLM struggles to respond.
...
We see these issues combined in OkHttp as the conflict is straightforward: parallel changes result in an object that is different from both branches, thus there is emergent behaviour. Simultaneously, the class is significantly more complex than the other subjects. Thus while in the PF examples, the possibility of a conflict is flat out dismissed,
...
To improve identification, further work should, on one hand, explore how to provide a better understanding of semantic conflicts, specifically that emergent behaviour is the result of a semantic conflict.
...
However, most cases were imprecisions and omissions, not entirely wrong descriptions. In some cases, these were the result of limitations of our prompt.
...
Another aspect worth considering is that while in many cases, ChatGPT could reliably describe which elements and interactions were the origin of the conflict, it would rarely then explain how that conflict manifested,
...
\end{response}
It is possible this is due to the very straightforward logic of the CM example. In particular, it is hard to describe the conflict without mentioning the relation between admin and the method throwing.
...
We observe Cart (CM) is fairly straightforward: if the user is an admin, checkout throws and exception. Indeed, even without any of the merge info, it would be fairly trivial to identify this is unintended, whereas
...
These come in the format b (for Base), A, B, and M (for Merge). C stands for compilation failures, F for failing test, and P for passing tests.
...
\begin{itemize}
  ...
  \item Tests are generally compilable or very close to successful compilation, with constructor and other errors, such as usage of non-existent getters as the main issues.
\end{itemize}
...
\begin{table}[t]
    ...
    \midrule
    \end{tabular}
    \caption{Results by conflict and explanation type, including successful conflict identification (after edits), the average edit distance to a compiling and passing test as well as the errors found.
...
Cart (CM) Manual            & & \textbf{F} & \textbf{F} & \textbf{F} & \textbf{P} & & \textbf{F} & \textbf{F} & \textbf{F} & \textbf{P} & & \textbf{F} & \textbf{F} & \textbf{F} & \textbf{P} \\
                            & &   &   &   &   & & P & P & P & P & &   &   &   &   \\
Cart (CM) ChatGPT           & & \textbf{\textit{F}} & \textbf{\textit{F}} & \textbf{\textit{F}} & \textbf{\textit{P}} & & \textbf{\textit{F}} & \textbf{\textit{F}} & \textbf{\textit{F}} & \textbf{\textit{P}} & & \textbf{F} & \textbf{F} & \textbf{F} & \textbf{P} \\ %italicize first two
                            & &   &   &   &   & &   &   &   &   & & F & F & P & P \\
...
Antlr (PF) Manual           & & \textbf{F} & \textbf{F} & \textbf{F} & \textbf{P} & & \textbf{F} & \textbf{F} & \textbf{F} & \textbf{P} & & \textbf{F} & \textbf{F} & \textbf{F} & \textbf{P} \\
    ...
    The cases in bold indicate successful tests.
    Several rows are found where multiple unit tests were generated.
    \label{tab:results:rq2tests}}
\end{table}


Analysis of \Cref{tab:results:rq2tests} allows us to identify in which cases the conflict was identified, and which ones it was not.
...
\end{itemize}
%
The cases that show that a semantic conflict is present require failure in all branches except merge, or compilation failure in base and one of the branches and failure on the other.
These are:
\begin{itemize}
  \item FFFP
  \item CCFP
...
the ChatGPT explanation of the conflict these asserts were more often correct. This is clearly an issue with the manually written explanation,
...
value and amount of Item are accessed. However, Item was frequently initialized with a constructor with three parameters: Name,
...
that for these complex real-world scenarios, aiding test generation by giving examples of existing test suites will be a necessary part of the prompt. Indeed, even if ChatGPT could generate
...
private methods. But it has to be understood that these sorts of issues are a possibility given the volatility of LLMs.



\section[Research Question 3: Existing prompts]{RQ3: Can state-of-the-art prompts (which do not explain the conflict) lead ChatGPT to generate
...
\begin{table}[t]
    ...
    \bottomrule
    \end{tabular}
    \caption{Results for Prompt 1, 2, 3 and 4, listing how many tests compile (or run without errors) and from those, how many tests pass. The trials row indicates how many times each prompt was run.\label{tab:results:rq3t1}}
\end{table}

%compilation/runtime

\Cref{tab:results:rq3t1} allows us to observe that issues relating to compilation and correct assertions remain
a problem for these prompts, confirming this is not necessarily just an issue with the prompt we developed.
%
...
in the prompt, we observed the adaptation of the \texttt{add\_item(Item)} method into one that took a list input, in Prompt 3:
\begin{response}
    @Before
    public void setup() {
...
which so far we had failed to get and where other prompts failed too, as they all sought to access the private method copyWithDefaults().
However, the two compilation failures for OkHttp in Prompt 3 also failed due to private access, but to a field rather than a method.

Prompt 4 suffered from limitations of generation and it would often stop generating mid-test. For our purposes these were cropped and not counted for the analysis.
...
\begin{table}[t]
    ...
    \bottomrule
    \end{tabular}
    \caption{Number of tests that identify the conflict, if simple corrections are made such as in RQ2. The trials row indicates how many times each prompt was run.\label{tab:results:rq3t2}}
\end{table}
...
The most significant difference, if we dismiss the specific case of Prompt 3, is that while we can achieve successful results,
there is much higher variability: in most cases for RQ2, if one trial could be amended, all three could be, while here we observe much
...
\end{response}
This is a perfectly valid test and it verifies several important features of the getBadWords method: it should
...
leading to fewer tests per generation. Furthermore, tests showed less variance, with variation being mostly on values, not the
...
\end{response}
It is worth noting that for this comparison we closely followed Prompt 2's format for 4, with the main variable being the usage of Copilot
...
in regard to compilation, might have been achieved if we had adopted some known good practices for Copilot generation, such as giving
...
\medskip
Overall, the response to RQ3 is yes, existing prompts can produce similar results to the prompt developed for RQ2
and can in some cases test the emergent behaviour. However, this comes with higher randomness and thus associated effort.
