...
\todo{Jose: I would suggest to organize this section in three subsection:
one to report the results regarding identification of the conflict,
another one to report whether ChatGPT understands the conflict,
and another to report whether ChatGPT is capable of explaining the conflict.}
...
\begin{itemize}
  ...
  \item \textbf{In none of the subject did the tool mistake the kind of conflict under discussion}\todo{I don't understand it, try to re-write it} ($B=1.0$). This might be because the prompt did not assert the existence of a conflict, allowing negative responses, whereas other prompts, which asked to find a conflict that did exist, lead the model to find any explanation, which often consisted of describing a textual conflict.

  \item \textbf{Conflict assertion remains uncertain}. ($C=0.9$) In many\todo{how many?} cases, particularly those of PF type, no conflict as\todo{was? were? has been?} identified. In many\todo{how many?} other cases, it was found but with uncertainty, e.g., with words such as ``possible'', and ``might''.
...
type of conflict, and evaluation type\todo{what?}.
These\todo{?} list the passing tests, after edits, the average edit distance to tests that compile and pass in merge,
and the errors that stop the conflict from being detected.

\Cref{tab:results:rq2tests} reports the values for the tests, after compilation errors are fixed in merge.\todo{don't understand it}
...
The first two results for Cart (CM) ChatGPT are italicized: these utilized deprecated features of Junit, and required rolling back to an earlier version to run.\todo{is this really relevant?}
OkHttp is omitted from the results as ChatGPT only generated import statements.
It is unknown why.
...
Antlr (PF) Manual           & & F & F & F & P & & F & F & F & P & & F & F & F & P \\
        \bottomrule
    \end{tabular}
    \caption{Test \todo{compilation and} running results in base (b), branch A,
    branch B, and merge (M).  C stands for compilation failures, R for runtime errors,
    F for failing test, and P for passing tests.
...
However, tests generated for \texttt{move} were always wrong\todo{misleading}; in turn these were just \texttt{x + distance} and \texttt{y + distance} additions.
...
\begin{table}[t]
    ...
    \bottomrule
    \end{tabular}
    \caption{Results for Prompt 1, 2, 3 and 4, for compilations and tests.\label{tab:results:rq3t1}}
\end{table}
