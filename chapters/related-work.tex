...
\todo{Can we call this section ``Detection with Testing''?}
...
%
\begin{itemize}
  \item \textbf{Changes-Matcher} module that identifies the possible presence of semantic conflicts, by first computing the changes between different versions (base and variants) and then comparing it to a set of patterns (listed in \Cref{table:pattern-table} \todo{o `)' fecha aqui?} describing common sources of conflicts as a base. From this it generates a DSL file, highlighting which methods and classes should be put under test to identify the conflict.
